...
with open(dirname + '/calibration_mod.yaml', 'r') as f:
...
# def Vel_to_PWM(linear_vel,angular_vel,bot_id):
	...
	w = config['pwm_left'].split(" ")
	w = np.array(list(map(float,w)))
	
	leftpwm = np.array([angular_vel,linear_vel,1])
	leftpwm = int(leftpwm.dot(w))
	print(leftpwm)
		if leftpwm > 255:
			leftpwm = 255
	
    w = config['pwm_right'].split(" ")
	w = np.array(list(map(float,w)))
	
	rightpwm = np.array([angular_vel,linear_vel,1])
	rightpwm = int(leftpwm.dot(w))
	print(rightpwm)
		if rightpwm > 255:
			rightpwm = 255
