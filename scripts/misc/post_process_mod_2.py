...
def process_data(dir_name):
    ...
    for filename in glob.glob(dir_name + '/calibration_files/pos_*.csv'):
        ...
        if not is_stationary(x_pos, y_pos):
            ...
            if motion == 'forward':
                pwm_left_forward.append([pwm, left_vel, right_vel])
                pwm_right_forward.append([pwm, right_vel, left_vel])

            elif motion == 'backward':
                pwm_left_backward.append([-pwm, left_vel, right_vel])
                pwm_right_backward.append([-pwm, right_vel, left_vel])
        else:
...
def regress(data):
    ...
    X = np.array([[c[1]** 2, c[1] , 1,c[2]] for c in data])
...
if __name__ == '__main__':
    ...
    with open(dirname + '/calibration_mod_2.yaml', 'w') as f:
